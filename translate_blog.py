...
FIELDS_TO_TRANSLATE = ['title', 'h1', 'h2', 'text', 'text1', 'text2', 'text3', 'text4', 'text5', 'text6', 'tipo']
...
def main():
    ...
    result_map = {item['slug']: item for item in translated}

    for i, entry in enumerate(source):
        slug = entry['slug']
        base = result_map.get(slug, entry.copy())
        ...
        for lang in TARGET_LANGS:
            ...
            for field in FIELDS_TO_TRANSLATE:
                if field not in base['translations'][lang] and entry.get(field):
                    ...
                    print(f"[{i+1}/{len(source)}] {slug} — {field} → {lang}: OK")
                    time.sleep(3)

        result_map[slug] = base
